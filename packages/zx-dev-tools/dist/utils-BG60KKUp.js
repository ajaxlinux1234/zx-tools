...
import fsPromises, { stat as stat$5 } from 'fs/promises';
import require$$0$1 from 'fs';
import require$$0 from 'constants';
import require$$1 from 'stream';
import require$$5 from 'assert';
import { fs as fs$i, chalk } from 'zx';
...
async function copyFile$2 (srcStat, src, dest, opts) {
...
const access = promisify(fs$i.access);
const mkdir = promisify(fs$i.mkdir);
const copyFile = promisify(fs$i.copyFile);
const readdir = promisify(fs$i.readdir);
const getPkg = () => {
  log(`process.cwd():${process.cwd()}`);
  const pkgStr = fs$i.readFileSync(path$c.join(process.cwd(), "package.json"), "utf-8");
...
async function copyFolder(source, destination) {
  ...
  for (const file of files) {
    const sourcePath = path$c.join(source, file);
    const destPath = path$c.join(destination, file);
    if (file === ".git") {
      continue;
    }
    try {
      const stats = await stat$5(sourcePath);
...
function fnLog(target, propertyKey, descriptor) {
  if (descriptor) {
...
function readDirFile(dir, options, level, list = []) {
  if (!fs$i.existsSync(dir)) {
    return list;
  }
  if (typeof level === "number" && level < 0) {
    return list;
  }
  const stat2 = fs$i.statSync(dir);
  const files = stat2.isDirectory() ? fs$i.readdirSync(dir) : [""];
  let ignorePath = options.ignorePath || [
    path$c.resolve(process.cwd(), ".git"),
    path$c.resolve(process.cwd(), "node_modules")
  ];
  if (typeof ignorePath === "string") {
    ignorePath = ignorePath.split(",");
  }
  ignorePath = ignorePath.filter((i) => i.trim()).map((i) => i.split(path$c.sep)).map((i) => i.join("/"));
  files.forEach((filename, index) => {
    let pathname = path$c.resolve(dir, filename);
    const stat3 = fs$i.statSync(pathname);
    pathname = pathname.split(path$c.sep).join("/");
    ...
    if (stat3.isDirectory()) {
      ...
      if (typeof options.onFile === "function") {
        const prePath = (files[index - 1] ? path$c.resolve(dir, files[index - 1]) : null) || "";
        const nextPath = (files[index + 1] ? path$c.resolve(dir, files[index + 1]) : null) || "";
...
async function replaceFileContent(filePath, searchValue, replaceValue) {
  try {
    const data = await fs$i.promises.readFile(filePath, "utf-8");
    const lines = data.split("\n");
    const updatedLines = (typeof searchValue === "function" ? lines.map((line) => searchValue(line)) : lines.map((line) => line.replace(searchValue, replaceValue || ""))).filter((one) => one !== " ");
    const updatedData = updatedLines.join("\n");
    await fs$i.promises.writeFile(filePath, updatedData, "utf-8");
  } catch (error) {
    console.error(`Error processing file ${filePath}:`, error);
  }
}
async function asyncReadDirFile(dir, options, level, list = []) {
  if (!(await fs$i.stat(dir)).isDirectory()) {
    return list;
  }
  if (typeof level === "number" && level < 0) {
    return list;
  ...
  ignorePath = ignorePath.filter((i) => i.trim()).map((i) => i.split(path$c.sep)).map((i) => i.join("/"));
  const files = await fs$i.readdir(dir);
  for (const filename of files) {
    let pathname = path$c.resolve(dir, filename);
    const stat2 = await fs$i.stat(pathname);
    pathname = pathname.split(path$c.sep).join("/");
    if (ignorePath.some((i) => pathname.includes(i))) {
      continue;
    }
    if (stat2.isDirectory()) {
      ...
      if (typeof options.onFile === "function") {
        const prePath = (files[files.indexOf(filename) - 1] ? path$c.resolve(dir, files[files.indexOf(filename) - 1]) : null) || "";
        const nextPath = (files[files.indexOf(filename) + 1] ? path$c.resolve(dir, files[files.indexOf(filename) + 1]) : null) || "";
...
export { Logger as L, __dirname as _, lib as a, asyncReadDirFile as b, createDirIfNotExists as c, copyFolder as d, replaceFileContent as e, commonjsGlobal as f, getPkg as g, getDefaultExportFromCjs as h, fnLog as i, log as l, readDirFile as r };
