  ...
  return {
    ...
    resolveId(id) {
      ...
      if (id.startsWith('dep-types/')) {
        const fileUrl = new URL(`./${id.slice('dep-types/'.length)}.d.ts`, depTypesDir);
    ...
    renderChunk(code, chunk) {
      if (chunk.fileName.startsWith('runtime') || chunk.fileName.startsWith('types.d-')) {
...
function validateRuntimeChunk(this: PluginContext, chunk: RenderedChunk) {
  for (const id of chunk.imports) {
    if (!id.startsWith('./') && !id.startsWith('../') && !id.startsWith('types.d')) {
...
function replaceConfusingTypeNames(this: PluginContext, code: string, chunk: RenderedChunk) {
  ...
  for (const modName in identifierReplacements) {
    const imp = imports.find((imp) => imp.specifier === modName && imp.imports.includes('{'));
